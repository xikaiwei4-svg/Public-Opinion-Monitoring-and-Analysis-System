  ...
  React.useEffect(() => {
    ...
    if (type === 'line') {
      ...
      option = {
        ...
        yAxis: { type: 'value' },
        series: [
          { name: '正面', type: 'bar', data: data.map(item => item.positive), itemStyle: { color: '#52c41a' } },
          { name: '中性', type: 'bar', data: data.map(item => item.neutral), itemStyle: { color: '#1890ff' } },
          { name: '负面', type: 'bar', data: data.map(item => item.negative), itemStyle: { color: '#f5222d' } }
        ]
      }
    }
