...
import { Typography, Row, Col, Card, Spin, Alert, Statistic, Divider, Button, Select, Modal, message } from 'antd'
...
import { DownloadOutlined, AlertOutlined, FireOutlined, SyncOutlined } from '@ant-design/icons'
...
function Dashboard() {
  ...
  const handleExportData = useCallback(() => {
    // 模拟数据导出功能
    setTimeout(() => {
      message.success(`已导出${selectedDays}天的数据报表`)
    }, 500)
  }, [selectedDays])

  // 查看话题详情
  const handleViewTopicDetail = useCallback((topic: any) => {
  ...
  return (
    <div>
      ...
      <Modal
        ...
        footer={[
          <Button key="close" onClick={() => setTopicModalVisible(false)}>
            关闭
          </Button>
        ]}
      >
        {selectedTopic && (
          <div>
            ...
            <p><strong>热度：</strong>{selectedTopic.heat_score}</p>
            <p><strong>关键词：</strong>{selectedTopic.keywords?.join(', ')}</p>
          </div>
        )}
      </Modal>
    </div>
